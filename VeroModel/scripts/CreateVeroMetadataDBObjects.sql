...
--CREATE TRIGGER SCHEMA_PROJECT_AFTER_INSERT_TRIGGER AFTER INSERT ON SCHEMA_PROJECT
-- REFERENCING NEW AS NEWROW
-- FOR EACH ROW MODE DB2SQL
--    UPDATE SCHEMA_PROJECT SET ID = LOWER(ID) WHERE ID = NEWROW.ID;
...
CREATE TABLE SCHEMA_TABLE_JOIN (
    ID                     VARCHAR(36) NOT NULL PRIMARY KEY,
    TABLE_LEFT             VARCHAR(36) REFERENCES SCHEMA_TABLE(ID),
    TABLE_RIGHT            VARCHAR(36) REFERENCES SCHEMA_TABLE(ID),
    JOIN_TYPE              INT,
    JOIN_EXPRESSION        VARCHAR(500),
    SCHEMA_REPORT_BLOCK_ID VARCHAR(36) REFERENCES SCHEMA_REPORT_BLOCK(ID),
    SCHEMA_QUERY_BLOCK_ID  VARCHAR(36) REFERENCES SCHEMA_QUERY_BLOCK(ID),
    LAST_MOD_TS            TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CREATION_TS            TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
